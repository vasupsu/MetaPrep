...
void init(int kLen, int numProcs, int numT)
{
	kmerSize=kLen;
	numP = numProcs;
	numThr = numT;
...
void getCanonKmers (char *readStr, uint32_t **kR, uint64_t *curSendOfs, int readLen, uint64_t startKmer, uint64_t endKmer, uint64_t kmerMask, int rotater, uint32_t curReadId, size_t *lastKmerAddedByThread, uint64_t *scanProcessKmerRange, int tid, int rank)
{
	int i,j;
	uint8_t lOfs[1024];
	...
	for (i=0; i<4; i++)
	{
		...
		{	
			if (canon[i]==0)
			{
				uint64_t curKmer = (uint64_t)key_h<<32;
				curKmer |= key_l;
				...
				if ((bucket >= startKmer) && (bucket < endKmer))
				{
					...
					process--;
					lOfs[process]++;
					lBuf[bufOfs].hi_kmer=key_h;
					lBuf[bufOfs].lo_kmer=key_l;
			...
			{
				uint64_t curKmer = (uint64_t)rc_h<<32;
                        	curKmer |= rc_l;
				uint64_t bucket=(curKmer & kmerMask) >> rotater;
				if ((bucket >= startKmer) && (bucket < endKmer))
				{
					...
					while (bucket >= scanProcessKmerRange[process])
						process++;
                                        process--;
					lOfs[process]++;
					lBuf[bufOfs].hi_kmer=rc_h;
					lBuf[bufOfs].lo_kmer=rc_l;
					lBuf[bufOfs].process=process;
					bufOfs++;
				}
			}
	...
	for (i=0; i<numP; i++)
	{
		if (lOfs[i] > 0)
		{
			...
			ptrs[i]=kR[i*numThr + tid] + curOfs*3;
	...
	for (i=0; i<bufOfs; i++)
	{
		...
		ptrs[p][0]=lBuf[i].lo_kmer;
		ptrs[p][1]=lBuf[i].hi_kmer;
		ptrs[p][2]=curReadId;
		ptrs[p]+=3;
	}
}
